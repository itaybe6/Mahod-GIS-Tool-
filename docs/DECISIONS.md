...
### נתוני תחבורה ציבורית (GTFS) — לא כלולים במשיכת הסוכן

מערך נתוני ה-**GTFS** של תחבורה ציבורית **לא** נכלל ב-`update-agent` ולא הוגדרה לו שורה ב-`data_sources` לצורך משיכה אוטומטית. הסיבות:

1. **גודל הקובץ.** ה-ZIP של GTFS ישראל שוקל **מאות MB** (שאר ה-ZIP שהגדרנו שוקלים מספר MB בלבד). הורדתו כולו לזיכרון של Edge Function (Deno, ~~512 MB RAM~~ 150–256 MB בפועל בסביבת Supabase) עלולה לגרום לקריסה מיידית של הסוכן — `OutOfMemory` באמצע הריצה לפני שנכתבה אפילו שורה אחת.

2. **מגבלת Supabase בתוכנית Free / Proללא TEAM.** העלאה של מיליוני שורות (`stop_times.txt` לבד מכיל ~10M+ שורות) ב-batch בודד של cron חודשי חורגת ממכסות ה-row count ומנגע ישיר ב-connection pool. בחשבון TEAM (עם `pg_bulk_insert` וקציבים גבוהים יותר) אפשר יהיה לשקול זאת מחדש.

3. **אסטרטגיה חלופית שכבר מיושמת.** הנתונים נטענו **ידנית** בהתחלה — הורדת ה-ZIP, פירוק לקבצים, והרצת סקריפטי `seed:*` (ראו `scripts/seed/`) שמעלים לטבלאות בשלבים. זו גישה מבוקרת יותר שמאפשרת בדיקה ידנית בין שלב לשלב ולא "כל-או-כלום" כמו ריצת cron.

**התוצאה בפועל:** ה-GTFS בפרויקט הוא snapshot ממועד ההקמה. כל עדכון עתידי יצריך הרצה ידנית של סקריפטי ה-seed — מה שמקובל כי לוחות זמנים של תחבורה ציבורית מתעדכנים בתדירות שמחייבת בדיקה איכותית ולא רק import עיוור.
