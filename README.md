...
> צילומי מסך של האפליקציה בפעולה — ראו **[docs/SCREENSHOTS.md](./docs/SCREENSHOTS.md)** (35 צילומים בתיקיית `docs/SCREENSHOTS/`).
...
**יישום הדרישות:** כל סעיפי הפרויקט והמטלות יושמו במלואם. בנוסף נוסף **סעיף מקורי**: משתמש יכול **להירשם ולהתחבר**; לאחר ההרשמה נשלח **מייל אימות** — יש לאשר אותו לפני שניתן להתחבר למערכת. ההתחברות מאפשרת למשתמש **להעלות קבצים ולשמור אותם**, כך שבפעם הבאה שיתחבר — הקבצים שהעלה בעבר יהיו זמינים לו מיד דרך מסך **«קבצים אחרונים»**, ללא צורך להעלות אותם מחדש. הקבצים נשמרים בצד השרת (Supabase Storage) ומשויכים לחשבון האישי של המשתמש — כך העבודה נשמרת בין כניסות ולא נאבדת עם רענון הדפדפן בלבד.
