import { Building2, Bus, CalendarDays, MapPin, Network, Route, Waypoints } from 'lucide-react';
...
const GTFS_TABLES_BEFORE_SHAPES: TableConfig[] = [
  {
    ...
    columns: [
      ...
      { key: 'agency_lang', label: 'שפה', type: 'text', width: 'w-20' },
      { key: 'agency_timezone', label: 'אזור זמן', type: 'text', width: 'w-36' },
      { key: 'agency_fare_url', label: 'אתר תעריפים', type: 'text', searchable: false },
      ...AUDIT_COLUMNS,
    ],
  },
...

  {
    ...
    columns: [
      { key: 'stop_id', label: 'מס׳ תחנה (פנימי)', type: 'number', width: 'w-32' },
      { key: 'stop_code', label: 'קוד תחנה', type: 'number', width: 'w-28' },
      ...
      { key: 'parent_station', label: 'תחנת אם', type: 'number', width: 'w-28' },
...

  {
    ...
    columns: [
      { key: 'service_id', label: 'מס׳ שירות', type: 'number', width: 'w-28' },
      {
        key: 'sunday',
        label: 'א׳',
        type: 'boolean',
      ...
      { key: 'start_date', label: 'תחילת תוקף', type: 'date', width: 'w-32', searchable: false },
      { key: 'end_date', label: 'סיום תוקף', type: 'date', width: 'w-32', searchable: false },
      ...AUDIT_COLUMNS,
    ],
  },

  {
    ...
    columns: [
      ...
      { key: 'route_id', label: 'מס׳ קו', type: 'number', width: 'w-28' },
      { key: 'service_id', label: 'מס׳ שירות', type: 'number', width: 'w-28' },
...
const GTFS_STOP_ROUTE: TableConfig = {
  name: 'gtfs_stop_route',
  label: 'שיוך תחנה ↔ קו',
  description: 'אילו קווים עוברים בכל תחנה (לפי כיוון).',
  icon: Network,
  defaultSort: [
    { key: 'stop_id', dir: 'asc' },
    { key: 'route_id', dir: 'asc' },
  ],
  rowKey: (row) => `sr-${key(row, 'stop_id', 'route_id', 'direction_id')}`,
  columns: [
    { key: 'stop_id', label: 'מס׳ תחנה', type: 'number', width: 'w-28' },
    { key: 'route_id', label: 'מס׳ קו', type: 'number', width: 'w-28' },
    {
      key: 'direction_id',
      label: 'כיוון',
...
export function buildGtfsTableList(shapesMode: GtfsShapesSchemaMode): TableConfig[] {
  if (!SHOW_GTFS_SHAPES_TAB) {
    return [...GTFS_TABLES_BEFORE_SHAPES, GTFS_STOP_ROUTE];
  }
  const shapes = shapesMode === 'point' ? GTFS_SHAPES_POINT : GTFS_SHAPES_LINE;
  return [...GTFS_TABLES_BEFORE_SHAPES, shapes, GTFS_STOP_ROUTE];
}
